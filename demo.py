...
def initialize_wham():
    ...
    if not initialized:
        ...
        cfg.merge_from_file(os.path.join(repo_path, 'configs', 'yamls', 'demo.yaml'))
...
def run(cfg,
    ...
    with torch.no_grad():
        if not (osp.exists(osp.join(output_pth, 'tracking_results.pth')) and
            ...
            if filter_frames:
                ...
                # Filter the tracking results based on the number of visible keypoints
                ...
                logger.info("[WHAM preprocess] Frame filtering is DISABLED (use --filter to enable).")
                length = len(tracking_results[0]['frame_id'])
    ...
    for subj in range(n_subjs):
        ...
        results_opencap[_id]['trans_world'] = pred['trans_world'].cpu().squeeze(0).numpy()
        results_opencap[_id]['betas']       = pred['betas'].cpu().squeeze(0).numpy()
        results_opencap[_id]['verts']       = (pred['verts_cam'] + pred['trans_cam'].unsqueeze(1)).cpu().numpy()
        results_opencap[_id]['frame_ids']   = frame_id
        results_opencap[_id]['contact']     = pred['contact'].cpu().squeeze(0).numpy()
        results_opencap[_id]['poses_body']  = pred['poses_body'].cpu().squeeze(0).numpy()
        results_opencap[_id]['poses_root_cam']   = pred['poses_root_cam'].cpu().squeeze(1).numpy()
        results_opencap[_id]['verts_cam']        = (pred['verts_cam'] + pred['trans_cam'].unsqueeze(1)).cpu().numpy()
        results_opencap[_id]['poses_root_world'] = pred['poses_root_world'].cpu().squeeze(0).numpy()
        results_opencap[_id]['trans_cam']        = pred['trans_cam'].cpu().squeeze(0).numpy()
        results_opencap[_id]['frame_id']         = frame_id
        results_opencap[_id]['cam_trans']        = pred['cam'].cpu().squeeze(0).numpy()
        results_opencap[_id]['tracking_results_for_reproj'] = tracking_results_reproj[_id]
        results_opencap[_id]['cam_R']            = pred['cam_R'].cpu().squeeze(0).numpy()
        results_opencap[_id]['cam_T']            = pred['cam_T'].cpu().squeeze(0).numpy()

    if save_pkl:
        joblib.dump(results,osp.join(output_pth, 'wham_output.pkl'))
        joblib.dump(results_opencap,osp.join(output_pth, 'wham_opencap.pkl'))
        logger.info(f'Saved wham_output.pkl and wham_opencap.pkl at {output_pth}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--filter', action='store_true',
                        default=False,
                        help=(
                            'Enable frame-level filtering: '
                            'filter_frames_by_bbox_height, filter_frames_by_bbox_touching_edges, '
                            'filter_frames_by_keypoints (and raises InsufficientFullBodyKeypointsError '
                            'if too few valid frames remain). Disabled by default.'
                        ))
